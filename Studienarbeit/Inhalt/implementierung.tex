...
\section{Animation}
In diesem Abschnitt der Arbeit wird auf die Implementierung der grafischen Ausgabe des neuronalen Netzwerks zur Erkennung von handgeschriebenen Zahlen in SetlX eingegangen. Das Ziel ist die grafische Aufbereitung der Daten aus dem Hauptprogramm. Es bildet die folgenden Informationen ab: 
\begin{itemize}
...
\vspace{0.5cm}
\noindent
Die folgenden Befehle sind zum ansteuern der einzelnen Funktionalitäten in der Anwendung.
\begin{center}
...
Die Eingabe für das Netzwerk ist eine handgeschriebene Zahl mit der Bildgröße von $28 \times 28$ Pixeln. Jeder Bildpixel ist eine Eingabe, wobei sich bei dieser Bildgröße 784 ergeben (siehe Abb. \ref{fig:MNIST-Matrix}). Diese werden für die Animation ebenfalls in der Form eines Quadrats abgebildet. Weiterhin ist der Aufbau und die Verbindungen zwischen den Neuronen der verborgenen Schicht und der Ausgabeschicht dargestellt. \\

\begin{figure}[hbt]
	\centering
	\includegraphics[scale=0.3]{Bilder/MNIST-Matrix}
...
\noindent
Die \textbf{Eingabebefehle 1 - 30} betrachten den Untersuchungsbereich eines einzelnen Neurons der verborgenen Schicht, welches hierbei bei der Auswahl farblich hervorgehoben wird. Der Untersuchungsbereich ist farblich in rote und blaue Bereiche aufgeteilt. Pixel die eine rote Färbung, haben eine hohe Gewichtung. Dahingegen weisen Pixel mit einer blauen Färbung eine geringen Gewichtungsfaktor auf. Blasser Farben haben hierbei eine geringere Merkmalsausprägung. Die Animation weist hierbei den folgenden Aufbau auf (siehe Abb. \ref{fig:untersuchungsbereich_neuron}).
...
\noindent
Über den \textbf{Eingabebefehl 1000} der Untersuchungsbereich aller Neuronen in einer Übersicht aufbereitet. Dies bedeutet, dass dieser Befehl alle Untersuchungsbereiche aus den Eingabebefehlen 1-30 in sich zusammenfasst (siehe Abb. \ref{fig:untersuchungsbereich_neuronen}). Somit ist wird dem Nutzer ermöglicht Rückschlüsse zu führen, welche Bereiche jedes einzelne Neuron mehr betrachtet und welche nicht. \\
...
\noindent
An dieser Stelle soll angemerkt sein, dass aufgrund der Verwendung von nur einer verborgenen Schicht in diesem Versuchsaufbau und der Anzahl von nur 30 Iterationen keine Erkenntnisse anhand der grafischen Ausgaben getroffen werden können. Hierzu müsste eine müsste ein \textit{Multilayer-Network} mit einer wesentlichen höheren Anzahl von Iterationen verwendet werden.
